...
public class Leaderboard extends Command{
    int pageSize = 10;
    public Leaderboard(CommandService commandService, Message message){
    ...
    public MessageEmbed createLeaderboardMessage(long guildID, int page){
        ...
        embedBuilder.setDescription("**"+guild.getName()+"'s Leaderboard**");
        ...
        for(Document m : leaderboard){
            ...
            int experience = m.get(Long.toString(guildID), Document.class).getInteger("EXPERIENCE");
            int level = Rank.getLevel(experience);
            String nickname;
            if(member!=null) {
            ...
            embedBuilder.addField("Rank " + rank + " - " + nickname, "Level " + level + " - " + experience + "xp", false);
            rank=rank+1;
        }
        return embedBuilder.build();
    }
