...
public class LevelAssignAdd extends Command{
    public LevelAssignAdd(CommandService commandService, Message message) {
    ...
    public void run(Message message){
        ...
        /* Now we should make sure the level requirement is actually a thing */
        int levelReq = 0;
        try{
            levelReq = Integer.parseInt(messageSplit[1]);
        }
        catch (NumberFormatException e){
            if(writePermissions)
                message.getChannel().sendMessage("Syntax Error: No level requirement specified.").queue();
            commandService.ongoingCommands.remove(this);
            return;
        }
        catch (Exception e){
        ...
        for(Role role : rolesByName){
            experienceService.setLevelRole(guildID, levelReq, role.getIdLong());
            addnum=addnum+1;
        }
        message.getChannel().sendMessage("Successfully added " + addnum + " roles to the level based auto assign service.").queue();
        commandService.ongoingCommands.remove(this);
    }
