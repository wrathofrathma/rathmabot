...
public class GuildRankUpdate extends TimedEvent {

    public Database database;
    public JDA jda;

    public GuildRankUpdate(JDA jda, Database database){
        this.jda = jda;
    ...
    public void checkGuildRankRoles(long guildID){
        ...
        if(!experienceService.rankRoles.containsKey(guildID))
            return;
        if(experienceService.rankRoles.get(guildID).isEmpty())
            return;

        Map<Long, Integer> roleMap=  experienceService.rankRoles.get(guildID);
        ...
        if(removalList.size()>0)
            experienceService.removeRankRoles(guildID, removalList);
        ...
        List<Document> leaderboard = utils.getSortedGuildMembers(database, guildID, "EXPERIENCE");
            ...
            for(Role r : roleList){
                ...
                if(member!=null){
                    ...
                    if((rank==0 || rank>rankReq) && memberRoles.contains(r)){
                        ...
                        System.out.println("Removing role \"" + r.getName() + "\" from rank " + rank + " - " + member.getEffectiveName());
                        guild.getController().removeRolesFromMember(member, r).queue();
                    }
                    else if(rank<=rankReq && !memberRoles.contains(r)){
                        System.out.println("Adding role \"" + r.getName() + "\" to rank " + rank + " - " + member.getEffectiveName());
